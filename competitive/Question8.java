...
The number of guests entering (E[i]) and leaving (L[i]) the party at every hour is represented as elements 
of the array. The task is to find the maximum number of guests present on the cruise at any given instance 
within T hours.
...
public class Question8 {
  ...
  public static void main(String[] args) {
    ...
    // Calculate the sum of differences and find the maximum
    for (int i = 0; i < t; i++) {
      int sum = 0;
      for (int j = 0; j < t; j++) {
        sum += E[i] - L[j];
